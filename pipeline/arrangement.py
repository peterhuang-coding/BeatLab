...
def _build_audio_events(
    ...
    *,
    ...
    for index, placement in enumerate(placements):
        ...
        # Store the effective renderer policy in the portable event itself.
        multiplier = 1.5 if track_id == "track-vocals" else 1.0
        placement["gain"] = common.clamp(float(placement.get("gain", 1.0)) * multiplier, 0, 1)
        if track_id == "track-vocals" and not placement.get("stretch_to"):
            placement["end_sec"] = min(float(placement.get("end_sec", 0)),
                                       float(placement.get("start_sec", 0)) + 8.0)
...
def build_arrangement(spec, recipe: dict, kit: dict[str, list[str]]) -> dict:
    ...
    return {
        ...
        "sample_rate": 44100,
        "audio_contract": {"processed_and_dry_format": "FLOAT", "dry_gain": "unchanged",
                           "premaster": "sum of dry stems; may exceed 0 dBFS",
                           "listening_mix": "tanh and common peak gain"},
